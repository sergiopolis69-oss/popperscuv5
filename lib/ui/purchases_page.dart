...
class _PurchasesPageState extends State<PurchasesPage> {
  ...
  double get totalMonto =>
      _items.fold(0.0, (a, b) => a + (b.quantity * b.unitCost));
  ...
  Future<void> _addProductBySku() async {
    ...
    if (p == null) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('SKU no encontrado')));
      return;
    }
  ...
  void _promptQtyPriceAndAdd(
      {required int productId, required double suggestedCost}) async {
    final qtyCtrl = TextEditingController(text: '1');
    final costCtrl = TextEditingController(
        text: suggestedCost > 0 ? suggestedCost.toStringAsFixed(2) : '');
    await showDialog(
        context: context,
        builder: (ctx) {
          return AlertDialog(
                    ...
                    if (q > 0 && c > 0) {
                      setState(() {
                        _items.add(PurchaseItem(
                            productId: productId, quantity: q, unitCost: c));
                      });
                      Navigator.pop(ctx);
                    }
                  },
  ...
  Future<void> _save() async {
    ...
    if (folio.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Folio requerido')));
      return;
    }
    if (_supplierId == null) {
      ScaffoldMessenger.of(context)
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Compra guardada #$id. Inventario actualizado.')));
  }
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(
                labelText: 'Folio de compra', prefixIcon: Icon(Icons.tag)),
          ),
          const SizedBox(height: 12),
          _SupplierLiveSearch(
              controller: _supplierCtrl, onSelected: (id) => _supplierId = id),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(
                child: TextField(
              controller: _skuCtrl,
              decoration: const InputDecoration(
                  labelText: 'Agregar por SKU',
                  prefixIcon: Icon(Icons.qr_code)),
              onSubmitted: (_) => _addProductBySku(),
            )),
            const SizedBox(width: 8),
            FilledButton.icon(
                onPressed: _addProductBySku,
          ...
          _ProductLiveSearch(
              ...
              onPickProduct: (prodId, lastCost) => _promptQtyPriceAndAdd(
                  productId: prodId, suggestedCost: lastCost ?? 0)),
          const SizedBox(height: 12),
          Card(
            ...
            ListTile(
                ...
                subtitle: Text(
                    'Piezas: $totalPiezas   |   Total: \$${totalMonto.toStringAsFixed(2)}')),
            const Divider(height: 1),
            ..._items.map((it) => ListTile(
                  dense: true,
                  title: Text('ID ${it.productId}  x${it.quantity}'),
                  subtitle: Text(
                      'Costo: \$${it.unitCost.toStringAsFixed(2)}   Importe: \$${(it.quantity * it.unitCost).toStringAsFixed(2)}'),
                  trailing: IconButton(
            ...
            if (_items.isEmpty)
              const Padding(
                  padding: EdgeInsets.all(12.0), child: Text('Sin productos aún')),
          ])),
          const SizedBox(height: 12),
          FilledButton.icon(
              onPressed: _save,
              icon: const Icon(Icons.save),
              label: const Text('Guardar compra y actualizar inventario')),
        ],
      ),
    );
  }
...
class _SupplierLiveSearchState extends State<_SupplierLiveSearch> {
  ...
  final FocusNode _focus = FocusNode(); // FIX: focus node añadido
  ...
  Future<void> _search(String q) async {
    ...
    setState(() =>
        _options = res.map((s) => MapEntry(s.id!, s.name)).toList());
  }
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      RawAutocomplete<MapEntry<int, String>>(
        ...
        focusNode: _focus, // FIX: focus node agregado
...
class _ProductLiveSearch extends StatefulWidget {
  ...
  const _ProductLiveSearch(
      {required this.controller, required this.onPickProduct});
  @override
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ..._results.take(6).map((row) {
        ...
        return ListTile(
          dense: true,
          title: Text(name),
          subtitle: Text(lastCost == null || lastCost == 0
              ? 'Último costo: —'
              : 'Último costo: \$${lastCost.toStringAsFixed(2)}'),
          trailing: IconButton(
              icon: const Icon(Icons.add),
              onPressed: () {
                widget.onPickProduct(id, lastCost);
              }),
        );
      }),
    ]);

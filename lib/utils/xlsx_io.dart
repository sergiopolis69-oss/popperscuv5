...
String _asString(ex.Data? d) {
  if (d == null) return '';
  if (d is ex.TextCellValue) return (d as ex.TextCellValue).value.text ?? '';
  if (d is ex.DoubleCellValue) return (d as ex.DoubleCellValue).value.toString();
  if (d is ex.IntCellValue) return (d as ex.IntCellValue).value.toString();
  if (d is ex.DateCellValue) {
    final dd = d as ex.DateCellValue;
    final dt = DateTime(dd.year, dd.month, dd.day);
...
double _asDouble(ex.Data? d) {
  ...
  if (d is ex.DoubleCellValue) return (d as ex.DoubleCellValue).value;
  if (d is ex.IntCellValue) return (d as ex.IntCellValue).value.toDouble();
  if (d is ex.TextCellValue) {
    final s = ((d as ex.TextCellValue).value.text ?? '').replaceAll(',', '.');
...
int _asInt(ex.Data? d) {
  if (d == null) return 0;
  if (d is ex.IntCellValue) return (d as ex.IntCellValue).value;
  if (d is ex.DoubleCellValue) return (d as ex.DoubleCellValue).value.round();
  if (d is ex.TextCellValue) return int.tryParse((d as ex.TextCellValue).value.text ?? '') ?? 0;
  return 0;
}
...
DateTime? _asDate(ex.Data? d) {
  ...
  if (d is ex.TextCellValue) return DateTime.tryParse((d as ex.TextCellValue).value.text ?? '');
  return null;
}
...
ex.CellValue _date(DateTime dt) => ex.DateCellValue(year: dt.year, month: dt.month, day: dt.day);
...
Future<Uint8List> buildProductsXlsxBytes() async {
  ...
  sh.appendRow(<ex.CellValue?>[
    _tx('sku'), _tx('name'), _tx('category'),
    _tx('default_sale_price'), _tx('last_purchase_price'), _tx('stock')
  ]);
  for (final r in rows) {
    sh.appendRow(<ex.CellValue?>[
      _tx((r['sku'] ?? '').toString()),
  ...
  await db.transaction((txn) async {
    ...
    for (final row in sh.rows) {
      ...
      if (pid == 0) {
        ...
        await txn.update('products', data,
